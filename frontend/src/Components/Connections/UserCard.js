    ...
    <div className="w-full bg-white rounded-lg border hover:bg-yellow-50/20 hover:border-yellow-500 shadow-lg p-6 mb-6">
      <div className="flex items-start space-x-6">
        <img
          ...
          alt={user.name || "User  profile image"} // Fallback alt text
          className="w-40 h-40 object-cover rounded-lg"
        />
        <div className="flex-1">
          <h3 className="text-xl font-bold text-gray-800">{user.name}</h3>
          <p className="text-sm text-gray-500">{user.degree}</p>
          <p className="text-sm text-gray-500">{user.techSkills?.length ? user.techSkills.join(", ") : "No skills listed"}</p> {/* Fallback for tech skills */}
        </div>
